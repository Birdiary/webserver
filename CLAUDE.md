...
`GET /api/movement/<station_id>` supports server-side `movements`/`offset`/`species`/`validation`/`undetected`/`date`/`days`/`from`+`to` for UI performance. `species` matches **any** detection in a movement (not just `detections[0]`). `validation=<latinName>` filters by validation entries (e.g. `validation=None` or `validation=Parus_major`). `undetected=true` filters for movements with no bird detected (latinName = "None"). Date-filter precedence: `from`/`to` → `days` → `date`. Species filter uses `_` for spaces (e.g. `Parus_major`). `GET /api/station/<station_id>` paginates movements and returns `movementsMeta` for the UI "load more" flow. The UI request layer for all of this is [nginx/data_visualization/src/helpers/requests.js](nginx/data_visualization/src/helpers/requests.js).
