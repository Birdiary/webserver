...
class OwnMap extends React.Component {
  ...
  render() {
    ...
    return (
      <Grid container>
        <Grid size={{ xs: this.state.open ? 0 : 12, md: this.state.open ? 0 : 12, lg: this.state.open ? 8 : 12 }} style={{ position: "relative" }}>
          {this.state.open ? "" :
            <Button variant="contained" color="secondary" style={{ top: 10, right: 10, position: "absolute", zIndex: "5000" }} onClick={this.handleClickOpen} >
                ...
                <Popup minWidth={90}>
                  ...
                  {movement ?
                    <span> {language[this.props.language]["map"]["lastMovement"]}{this.displayTimestamp(marker.lastMovement.start_date)} <br></br>
                      {marker.lastMovement.video != "pending" ? <ReactPlayer playsInline src={marker.lastMovement.video} loop={true} controls={true} style={{ width: '100%', aspectRatio: '1 / 1' }} playing={true} /> : ""}
        ...
        {this.state.open ?
          <Grid size={{ xs: this.state.open ? 12 : 0, md: this.state.open ? 12 : 0, lg: this.state.open ? 4 : 0 }} style={{ maxHeight: "calc(100vh - (3rem + 64px))" }}>
            ...
            <h1 style={{ textAlign: "center", color: "var(--mui-palette-primary-main, #2E7D5B)" }}> {language[this.props.language]["dashboard"]["header"]}</h1>
