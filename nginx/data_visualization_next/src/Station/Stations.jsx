...
import SwipeDetector from '../helpers/SwipeDetector';
              ...
              <Box sx={{ bgcolor: 'background.paper' }} style={{ maxWidth: "94vw", marginLeft: "3vw" }}>
                ...
                >
                  {data.measurements.movements.map((movement, i) =>
                    <Tab key={i} value={i} label={displayTimestamp(movement.start_date)}></Tab>
                  )}
                </TabList>
              </Box>
              {data.measurements.movements.map((movement, i) =>
                <TabPanel key={i} value={i}>
                  <SwipeDetector
                onSwipeLeft={() => setValue(Math.min(data.measurements.movements.length - 1, value+1))}
                  ...
                  <Grid container spacing={4}>
                    <Grid size={{ lg: 8 }}>
                      {movement.video == "pending" ? < div><p>{language[props.language]["stations"]["wait1"]}<br />  </p> <Button variant="contained" onClick={() => { getStation() }} style={{ margin: "15px" }}>Refresh</Button></div>
                        :
                        <ReactPlayer playing={true} playsInline src={movement.video} loop={true} controls={true} style={{ width: '100%', height: 'min(95vw, 80vh)', aspectRatio: 1 }} />}
                    </Grid>
                    <Grid size={{ lg: 4 }}>
                      ...
                      <audio src={movement.audio} controls />
          ...
          <div className="station-special-birds">
            ...
            <Grid container spacing={2}>
              {specialBirds.map((bird, index) => (
                <Grid size={{ lg: 3, md: 4, sm: 6, xs: 12 }} key={`station-special-${bird.latinName || index}`}>
