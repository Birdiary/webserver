...
class CreateStation extends React.Component {
  constructor(props) {
    ...
    this.secretIV = import.meta.env.VITE_SECRET_IV;
  }
  ...
  handleNext = () => {
    this.setState((s) => ({ activeStep: Math.min(s.activeStep + 1, 2) }));
  };

  handleBack = () => {
    this.setState((s) => ({ activeStep: Math.max(s.activeStep - 1, 0) }));
  };
  ...
  render() {
    ...
    const wizard = createCopy.wizard || language.en.createStation.wizard
    const steps = [wizard["step1"], wizard["step2"], wizard["step3"]]
    const step1EmailOk = ownerEmail ? true : (!emailMissing && !emailInvalid)
    return (
      <div className="create-station-page">
        <div className="create-station-form">
        <Stepper activeStep={this.state.activeStep} alternativeLabel className="create-station-stepper">
          {steps.map((label) => (
            <Step key={label}><StepLabel>{label}</StepLabel></Step>
          ))}
        </Stepper>

        <div className="wizard-step" style={{ display: this.state.activeStep === 0 ? 'block' : 'none' }}>
        <div className="wizard-cols">
        <div className="wizard-col-main">
        ...
        />
        <FormControl className="wizard-type" style={{ width: "50vw" }}>
          <FormLabel id="demo-row-radio-buttons-group-label">{createCopy["type"]}</FormLabel>
          <span className="wizard-type-helper">{createCopy["typeHelper"]}</span>
          <RadioGroup
            row
        ...
        <div className="wizard-step" style={{ display: this.state.activeStep === 1 ? 'block' : 'none' }}>
        <p className="wizard-hint">{wizard["step2Hint"]}</p>
        <h4>{createCopy["stationConfig"]}</h4>
        <p className="wizard-config-intro">{createCopy["stationConfigText"]}</p>
        ...
        <div className="wizard-step" style={{ display: this.state.activeStep === 2 ? 'block' : 'none' }}>
        <p className="wizard-hint">{wizard["step3Intro"]}</p>
        <dl className="wizard-summary">
          <div className="wizard-summary__row"><dt>{wizard["summaryName"]}</dt><dd>{this.state.name || wizard["summaryNotSet"]}</dd></div>
          <div className="wizard-summary__row"><dt>{wizard["summaryEmail"]}</dt><dd>{effectiveEmail || wizard["summaryNotSet"]}</dd></div>
          <div className="wizard-summary__row"><dt>{wizard["summaryLocation"]}</dt><dd>{Number(this.state.position.lat).toFixed(4)}, {Number(this.state.position.lng).toFixed(4)}</dd></div>
          <div className="wizard-summary__row"><dt>{wizard["summaryType"]}</dt><dd>{this.state.type}</dd></div>
          <div className="wizard-summary__row"><dt>{wizard["summarySoftware"]}</dt><dd>{softwareLabels[this.state.stationSoftware] || this.state.stationSoftware}</dd></div>
        </dl>
        <FormControlLabel className="wizard-privacy" control={<Checkbox checked={this.state.checked} onChange={this.handleChecked} />} label={createCopy["dataPrivacyText"]} />
        {this.state.checked ? "" : <p className="wizard-privacy-note">{createCopy["dataPrivacy"]}</p>}
        </div>

        <div className="wizard-nav">
          <Button onClick={this.handleBack} disabled={this.state.activeStep === 0}>{wizard["back"]}</Button>
          {this.state.activeStep < 2
            ? <Button variant="contained" onClick={this.handleNext} disabled={this.state.activeStep === 0 && !step1EmailOk}>{wizard["next"]}</Button>
            : <Button color="primary" variant="contained" type="submit" onClick={this.sendData} disabled={!canSubmit}>{createCopy["submit"]}</Button>}
        </div>
        </div>
